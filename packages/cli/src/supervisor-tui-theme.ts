...
function decorateDockToken(
  ...
  if (token === '◆ FOCUS WORKSPACE' || token === '◆ DECISION GATE' || /^[◆◇●≋✦?] (?:OVERVIEW|INBOX|BOX|CONNECTIONS|CONN|FLEET|RUNTIME|RUN|LOGS|DOCTOR|HELP|SETUP|SOURCE|PROJECTS|RELEASE|TRANSFER|CONFIRMATION)$/u.test(token)) {
