...
describe('theme color decisions', () => {
  it('keeps agent suggestions separate from reviewed final decisions', () => {
    ...
    const evidence: ThemeColorEvidenceBundle = { schemaVersion: 1, sourceCommit: 'abc', staticManifestSchemaVersion: 1, runtimeBindingSchemaVersion: 1, playwrightVersion: '1', browserVersion: '1', images: [{ scenarioId: 'panel', theme: 'light', state: 'normal', relativePath: 'panel.jpg', sha256: 'hash', format: 'jpeg', quality: 80, width: 10, height: 10, viewport: { width: 10, height: 10 }, deviceScaleFactor: 1, inventoryIds: ['runtime'] }] }
  ...
  it('does not promote an agent suggestion by merely labeling it reviewed', () => {
    ...
    const input = buildAnalysisBundle(staticManifest, { schemaVersion: 1, sourceCommit: 'abc', bindings: [] }, { schemaVersion: 1, sourceCommit: 'abc', staticManifestSchemaVersion: 1, runtimeBindingSchemaVersion: 1, playwrightVersion: '1', browserVersion: '1', images: [] })
    const suggestions = analyze(input)
