...
async function relocateForCapture(event: RuntimeCaptureEvent): Promise<RuntimeCaptureEvent> {
  const direct = event.page.locator(`[data-openalice-color-audit~="${event.binding.inventoryId}"], .openalice-audit-${event.binding.inventoryId}`).filter({ visible: true }).first()
  const locator = await direct.count() > 0 ? direct : event.page.locator(event.binding.target.selector).filter({ visible: true }).first()
  if (await locator.count() === 0) return event
  await locator.evaluate((element) => element.scrollIntoView({ block: 'center', inline: 'center' })); await event.page.waitForTimeout(20)
