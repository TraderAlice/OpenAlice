...
const winner = (value: unknown, label: string): RuntimeWinnerProof => {
  ...
  if (kind === 'iframe-computed-style') {
    const computedProperty = text(input.computedProperty, `${label}.computedProperty`)
    if (computedProperty !== 'background-color' && computedProperty !== 'color') throw new Error(`${label}.computedProperty is unsupported`)
    return { kind, sourceValue: text(input.sourceValue, `${label}.sourceValue`), computedProperty, isolatedValue: text(input.isolatedValue, `${label}.isolatedValue`) }
  }
