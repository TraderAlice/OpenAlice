...
const worklist: RuntimeColorWorklist = { schemaVersion: 1, sourceCommit: 'abc', items: [{ inventoryId: source.inventoryId, source }] }
...
describe('occurrence-driven scenario coverage', () => {
  it('accepts an explicit inventory assignment', () => expect(() => validateScenarioCoverage(worklist, [scenario])).not.toThrow())
  it('rejects missing and stale IDs', () => {
    expect(() => validateScenarioCoverage(worklist, [])).toThrow('without scenarios')
