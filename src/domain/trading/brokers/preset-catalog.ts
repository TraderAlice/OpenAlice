...
export type BrokerEngine = 'ccxt' | 'alpaca' | 'ibkr' | 'iol'
...
export const IOL_PRESET: BrokerPresetDef = {
  id: 'iol',
  label: 'InvertirOnline (IOL)',
  description: 'Argentine retail broker — BYMA equities, CEDEARs, sovereign bonds, FCIs, and US markets via CEDEAR proxy.',
  category: 'securities',
  hint: `Authentication uses your IOL username and password (OAuth2 password grant).
Credentials can be supplied as literals, as "$env:MY_VAR" references, or left blank to read IOL_USERNAME / IOL_PASSWORD from environment variables.
Enable "Dry-run" to verify wiring without placing real orders.`,
  defaultName: 'iol-main',
  badge: 'IOL',
  badgeColor: 'text-green-400',
  engine: 'iol',
  guardCategory: 'securities',
  zodSchema: z.object({
    username: z.string().default('').describe('Username / Email'),
    password: z.string().default('').describe('Password'),
    market: z.string().default('bCBA').describe('Default market (bCBA, nyse, nasdaq…)'),
    sandbox: z.boolean().default(false).describe('Dry-run mode (no real orders)'),
  }),
  subtitleFields: [
    { field: 'market', prefix: 'IOL · ' },
