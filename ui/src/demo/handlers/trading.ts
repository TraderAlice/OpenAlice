...
export const tradingHandlers = [
  ...
  http.get('/api/trading/config/broker-presets', ({ request }) => HttpResponse.json({
    presets: ['broker-picker', 'broker-conflict', 'broker-pack-missing', 'first-run-no-uta'].includes(auditFixture(request) ?? '')
      ? [auditBrokerPreset]
      : demoBrokerPresets,
  })),
  http.get('/api/trading/config/broker-packs', ({ request }) => {
    const fixture = auditFixture(request)
    const auditIbkrInstalled = ['broker-picker', 'broker-conflict', 'first-run-no-uta'].includes(fixture ?? '')
    return HttpResponse.json({
      packs: [
        ...
        {
          engine: 'ibkr',
          installed: auditIbkrInstalled,
          source: auditIbkrInstalled ? 'workspace' : 'missing',
          ...(auditIbkrInstalled ? { version: 'audit' } : {}),
          ...(fixture === 'broker-pack-missing' ? { reason: 'Audit fixture: optional IBKR support is not installed.' } : {}),
          requiredBy: fixture === 'broker-pack-missing' ? ['Demo IBKR'] : [],
        },
